...
**三个容易混淆的问题。** “从 A 能到 B”只要求存在一条路径；“A 支配 B”要求从入口到 B 的每条路径都经过 A；“A 直接支配 B”还要求它是离 B 最近的严格支配者。以入口 R 分别到 L、M，再汇合到 J 为例：L 能到 J，但 L 不支配 J，因为 R→M→J 绕过 L；R 支配 J，且在这个简单图中直接支配 J。

支配树的 R→J 边表示不可绕过的关系，不要求 CFG 中存在 R→J 这条边。反过来，CFG 中 L→J 的边也不要求出现在支配树中。原书把 CFG 与支配树并排画，正是为了比较这两种边的含义。

同一块内的指令还要考虑先后次序：一个块支配自身，并不意味着块尾的定义可以供块首使用。讨论指令支配时，块内通常按指令顺序判断；PHI 输入则按前驱边上的使用位置判断。把基本块支配结论直接套到每条指令，会错过这两个条件。
...
两图使用同一组节点，与下表和 graph7.ll 一致。例如 CFG 中没有 2→6 这条边，支配树中仍有 2→6。
...
实线是 CFG 边，虚线说明分析关系：3/4 的定义在 6 汇合；3/4 的执行受 2 的条件控制。支配边界与控制依赖使用的方向不同。
...
**先手算一轮，再读高效算法。** 对入口 R、分支 L/M、汇合 J，初始化 Dom(R)={R}，其他可达块为全集 {R,L,M,J}。采用 `Dom(B)={B} ∪ ⋂Dom(P)`，先处理 L、M，得到 {R,L}、{R,M}；再处理 J，取两者交集 {R}，加自身得到 {R,J}。这里交集表达“每个前驱路径都成立”，与活跃性只需某条路径使用值所以取并集的逻辑不同。

若在 L 定义 x、M 也定义 x，J 中使用 x，那么 J 属于 L、M 的支配边界。L 支配自己的出口，却不能支配 J；于是定义影响范围在 J 汇合。若还有后续汇合，J 中新产生的 PHI 定义又要作为新的起点计算边界，因此第 2 章的插 PHI 过程需要 IDF，而不只算一次 DF。

本节后面的 Semi-NCA、DFS 编号与半支配者，是在更低成本下计算同一支配关系的实现方法。阅读时先用这个集合方程核对最终关系，再追踪算法的 parent、semi、label 等中间量；这些数组项不能简单都解释成直接支配者。
...
```mermaid
flowchart TD
 A["a: DFS 进入 1，离开 8"] --> B["b: 进入 2，离开 7"]
 B --> C["c: 进入 3，离开 4"]
 B --> D["d: 进入 5，离开 6"]
```

这是对支配树做 DFS 的教学编号，不是对任意 CFG 的 DFS 编号。b 的区间包含 c/d，故 b 支配它们；c 与 d 的区间互不包含，故两者互不支配。具体实现可采用不同起始编号。
