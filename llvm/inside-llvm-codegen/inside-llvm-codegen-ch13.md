...
**把前十二章串成一条最小可工作的路径。** 新后端先要能接收一个返回常量或简单整数加法的函数：描述寄存器和指令，让选择器生成目标操作；实现形参与返回约定，让函数与调用者交换值；提供分配器需要的寄存器约束；最终打印或编码这些操作。接着引入局部栈对象和函数调用，才能暴露帧索引消除、保存恢复与更完整 ABI 的问题。

一种便于定位错误的递进顺序是：常量返回→两个形参加法→条件分支→load/store→局部栈对象→调用其他函数→高寄存器压力触发 spill。每增加一项，都能指出新需要哪个部件，而不是一次放入大程序、失败后从全部后端文件里搜索。

| 输入新能力 | 主要新增工作 | 成功时应观察什么 |
| --- | --- | --- |
| 常量与加法 | 指令/寄存器描述、选择、返回 lowering | 正确目标 opcode 与返回位置 |
| 分支 | 分支分析/插入、目标标签 | CFG 与条件方向正确 |
| 访存 | 宽度、对齐、地址模式 | 访问的字节数与地址保持正确 |
| 栈对象与 spill | FrameLowering、FI 消除、存取钩子 | 无残留 FI，栈访问与保存恢复合理 |
| 对象输出 | MC 编码、fixup、对象 writer | 字节、符号和重定位一致 |

这些步骤是学习与验收顺序，不替代各目标的完整规范。下文仍按原书章节顺序逐一介绍适配点，读者可用这张表判断每个文件解决哪一类输入失败。
...
**从一个失败用例决定该改哪里。** 若 `add i32` 在合法化阶段失败，先看目标为 i32 与 ADD 设置了什么类型/操作动作，不能只在指令描述里追加一个 Pattern。若合法化后已经是可支持的节点，却没有匹配，才追到 DAG selector 或生成 matcher 的形状与谓词。若选出指令后 MachineVerifier 报寄存器类不兼容，再核对输入输出约束。

函数调用另走 ABI 的语义链：LowerFormalArguments 决定被调用者如何获得输入，LowerCall 决定调用者如何放参数及获取结果，LowerReturn 决定返回位置。这三者必须一致。只让无参数加法函数生成汇编，不足以证明参数、返回扩展或跨函数保存规则已经正确。
...
先让局部对象在 MachineFrameInfo 中获得身份和布局约束，再由后续阶段确定偏移并消除 frame index。例如 MIR 的“访问 FI#0 内偏移 4”最终要变成“基址寄存器加对象偏移再加 4”；若立即数范围不足，还要生成合法地址计算。这个过程中不能把对象编号 0 误当字节偏移 0。

spill/reload、callee-saved 和普通局部变量都可能访问栈，但来源不同：前者由分配器按压力产生，中者由 ABI 与实际寄存器使用产生，后者来自程序对象。实现时用一个有调用且压力足够触发 spill 的小函数，逐项追踪每个槽的来源，会比只看最终 SP 调整总数更容易发现遗漏。
...
能够打印汇编是一个中间里程碑。接下来还要让相同 opcode/操作数得到正确编码，并处理常量、局部跳转、外部符号等不同表达式。先用无重定位的简单指令核对字节字段，再加本地分支检查布局修正，最后加外部引用检查 relocation，能把编码与符号解析问题分开。

如果打印的汇编正确而对象反汇编错误，重点检查编码字段、寄存器编号映射和 endian；如果指令本身正确但链接后地址错误，继续核对 fixup 种类、加数与重定位计算规则。目标注册成功只表示框架能找到相应实现，不替代这些行为验证。
