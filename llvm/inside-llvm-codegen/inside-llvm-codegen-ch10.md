...
**先明确要分配的是“同时需要保存的值”。** 假设只有 R0、R1 两个可分配寄存器，程序依次计算 a、b，再计算 c=a+b，随后计算 d=c+1。a、b 在加法前要同时存在，因此不能占同一位置；加法以后若两者都不再用，c 可以复用其中一个位置。d 同理可以复用 c 的位置。四个名字并不需要四个永久寄存器。

若再增加一个 x，要求从 a 之前一直保存到 d 之后，就可能同时需要 a、b、x 三个值。此时两个寄存器装不下，必须改变保存策略：把某个值放到栈里，在需要时读回；拆分某个长区间，只在热点或使用附近留在寄存器；或者在代价低且语义允许时重新计算值。分配器不是只给名字着色，还会改写指令并重新分析新值。

```mermaid
flowchart TD
 V["虚拟寄存器及定义使用"] --> L["计算活跃性与目标约束"]
 L --> A{"存在可用物理位置？"}
 A -->|是| M["建立分配"]
 A -->|否| E["剔除、拆分、溢出或重新物化"]
 E --> N["新增复制、访存或计算<br/>产生新的短区间"]
 N --> L
 M --> W["重写机器操作数"]
 W --> F["后续栈帧和机器代码处理"]
```

沿图阅读本章各 Pass：PHI 消除把边上的合并转为复制，二地址变换表达输入输出共用位置的限制，活跃区间提供冲突范围，合并减少复制，分配算法决定位置与必要改写，最后重写虚拟操作数。只有把这些步骤接起来，后面的 Fast、Basic、Greedy、PBQP 才是在比较解决办法，而不是四组互不相关的名称。
...
```mermaid
flowchart LR
 P["PHIElimination"] --> T["TwoAddressInstruction"]
 T --> F["Fast 寄存器分配与重写"]
```

这里只展示原书的三项主干；完整目标流水线仍可能包含前后钩子与其他处理。
...
```mermaid
flowchart BT
 U["使用 %3"] --> E["%3 = EXTRACT_SUBREG %2, sub1"]
 E --> R["%2 = REG_SEQUENCE<br/>只需 sub1"]
 R --> B["%1: 对应 sub1，有使用需求"]
 R -. "sub0 无此使用需求" .-> A["%0: 对应 sub0"]
```

箭头表示使用需求向定义的反向传播。是否可以删 %0 的定义指令还需检查其他使用及副作用。

**图 10-4 寄存器使用信息分析**

```mermaid
flowchart TD
 A["%0: 已定义值"] -->|sub0| R["%2: sub0 已定义，sub1 未定义"]
 B["%1: IMPLICIT_DEF"] -->|sub1| R
 R -->|"提取 sub1"| E["%3: 未定义值"]
```

箭头表示已定义/未定义信息向使用者传播；不能把整个 %2 都标成没有已定义 lane。
...
**用一段代码区分 kill 与 dead。** 教学指令为 `a=load p; b=a+1; c=b+2; store c,q`，假设 a、b、c 都无其他使用。第二条中 a 的 use 是最后使用，可以标 kill；第三条中 b 的 use 同理；最后 store 中 c 的 use 同理。它们都是“读完旧值后无需保存”，并不表示当前指令可以删。

如果另外有 `d=add u,v`，d 完全不被使用，则 d 的 def 可以是 dead：这个新值从产生起就没有后续需求。但只有当整条指令的副作用等条件允许时，才可删除指令。若是一条具有必须保留效果的指令附带死结果，结果 dead 仍不等于操作 dead。

再把代码放进分支：a 在入口定义，左支使用，右支不用。a 需要沿左边活跃，却不需要沿右边保持到出口。单纯把首次定义到源文件最后一次使用连成一整段，会把互斥分支也算成冲突。LiveVariables 与 LiveIntervals 采用不同表示，但都必须忠实反映 CFG 上实际的活跃路径。
...
**从一条 PHI 推到机器复制。** 设 J 有 `r=PHI(a,P,b,Q)`，P/Q 都无其他后继。直接的教学实现是在 P 末尾复制 a 到用于合并的临时位置 t，在 Q 末尾复制 b 到 t，然后在 J 用 t 得到 r。P、Q 两个定义位于互斥路径，执行到 J 时恰好只有对应路径的值生效。机器表示因而可以不再保持每个虚拟名唯一静态定义。

每一步都需要更新活跃性：原来 a/b 是边使用，现在复制指令产生显式使用；t 从前驱复制处活跃到 J 的使用处；r 的后续使用仍保留。若只删除 PHI 而没有维护这些关系，分配器就可能错误复用某个仍要保存的位置。

多个 PHI 必须同时考虑复制集合，第 2 章的并行复制与临界边问题仍然存在。LLVM 具体实现还利用 SSA 性质、临时虚拟寄存器和后续合并来处理这些复制；不要从一条简单 PHI 的成功例子，推出任意位置逐条赋值都能实现整组 PHI。
...
**为什么三地址加法前面可能多一条 COPY。** 三地址形式 `c=a+b` 允许 c 与两输入不同；目标若只有“把第二输入加到第一个输入并写回”的形式，需要令结果与一个输入占同一位置。最直接的表达是 `c=COPY a; c=ADD c,b`，第二条的 c 输入和 c 输出被绑定。

若 a 在此后不再使用，c 与 a 的区间可能合并，使 COPY 最终消失；若 a 还要使用，就不能直接破坏 a，必须保留复制或选择其他合法变换。对可交换加法，若 b 反而是最后一次使用，可以尝试交换输入，让结果复用 b。但减法等不可交换操作不能照做。

| a 后面还用？ | b 后面还用？ | 教学上的选择空间 |
| --- | --- | --- |
| 否 | 是 | 优先考虑复用 a 的位置 |
| 是 | 否 | 可交换操作可考虑复用 b |
| 是 | 是 | 两旧值均需保留，往往需要新复制或额外处理 |
| 否 | 否 | 两边都可能复用，再按约束和成本选择 |

这张表只解释值保存需求，实际还要检查寄存器类、tied operand、early-clobber 和目标变体。原书后面的指令交换、局部移动和三地址变体，都是为了在保持这些条件时减少复制、缩短重叠，而不是只把 opcode 名字换掉。
...
```mermaid
flowchart LR
 O["原顺序<br/>MI 定义 v<br/>其他指令<br/>KilledMI 最后使用 v"] --> A["方案一<br/>合法时把 MI 下移<br/>靠近 KilledMI"]
 O --> B["方案二<br/>合法时把 KilledMI 上移<br/>靠近 MI"]
```

两种移动都试图缩短 v 的活跃跨度；只有跨越指令的依赖、内存效果及目标约束允许时才可执行。
...
一条机器指令既可能读取旧值，又可能定义新值，所以“第 10 条指令”这个整数位置有时不够。若输入最后使用与输出定义恰好发生在同一条指令，能否共用位置还取决于 tied/early-clobber 等约束。SlotIndex 为指令附近提供更细的位置，方便比较区间端点以及特殊定义时机。

先用简化坐标理解半开区间：a 活跃于 [2,6)，b 活跃于 [6,9)，两者在 6 处首尾相接但不重叠，抽象上可复用位置；若 a 是 [2,7)，则与 b 的 [6,9) 在 [6,7) 重叠。下一个小节的范围计算建立在这种端点约定上。简化数字用于推导，实际 SlotIndex 的子位置必须按 LLVM 定义读取，不能把清单中的所有索引当成普通源代码行号。
...
**从每个使用点向定义回推，而不是把名字圈成一条长线。** 在线性教学序列中，a 在位置 1 定义，在 3、7 使用；b 在 2 定义，在 4 使用。a 必须从定义保存到最后使用，b 在它内部活跃，因此有干涉。若 a 在 4 之后被重新定义成另一个值，后面的 a 已不是前面的内容，就需要区分值号与各自 segment，不能因为名字相同而强行合并。

分支还会产生区间空洞。假设入口定义 a，只在左分支使用，右分支有大量计算，布局上右分支恰好排在入口和左分支之间。按线性编号看，a 的首尾跨度很长；按真实控制流，它未必在整段右分支中活跃。LiveInterval 可以由多个 segment 组成，干涉查询应比较这些实际段，而不是只比较最小开始与最大结束。

| 对象 | 示例 | 表达的信息 |
| --- | --- | --- |
| 虚拟寄存器名 | v | 指令操作数引用的名字 |
| 值号 | v 的某次定义 | 当前保存的是哪次定义的值 |
| segment | [start,end) | 这个值在哪一段位置必须存在 |
| interval | 多个 segment 的集合 | 分配与查询所处理的整体范围 |

寄存器分配之后，多个值也可能先后使用同一物理寄存器；合并、拆分和重写必须同时维护“名字”“值”“位置”的关系。这是本章最值得花时间区分的三件事。
...
**复制为什么有机会变成零条指令。** 对 `b=COPY a`，如果后面只使用 b 而 a 不再使用，给 a、b 分配同一物理位置，就已经满足复制语义，无需机器 COPY。合并器尝试把这种位置相等关系提前建立，减少分配器需要处理的复制。

但若 a 后面还要保留原值，b 又会被修改，两个名字的存储需求就可能冲突。寄存器类也要兼容：源只允许一组位置，目的只允许另一组，合并后的可选集合必须能同时满足两者。区间合并可能扩大某个节点的干涉范围，所以“复制越少越好”不能不加条件地作为唯一目标。

后面例子中的合并成功，应逐项看：合并前两值的定义使用是什么；复制点处是否同值；之后哪些段重叠；重叠是否可解释为同一个值；合并后寄存器类是否仍合法。这样才能理解为何两个外观相似的 COPY，一个被删，另一个保留。
...
**执行次数为何会改变溢出选择。** 两个区间长度相同、各有一次读取，一个位于只执行一次的冷块，另一个位于预计执行一千次的循环体。若溢出后在使用前增加 reload，静态都只多一条指令，动态代价却可能相差很大。块频率让分配器能估算这种差别。

教学上令一次 reload 成本为 1，冷块频率 1，热块频率 1000，则加权成本分别为 1、1000。真实 spill weight 还会结合定义/使用、区间大小、可重新物化和目标信息；这个乘法只解释频率为何进入模型。频率通常是估计或 profile 推导的相对量，并不保证循环真的运行恰好一千次。
...
**溢出到栈不表示每个值永久拥有一个独立槽。** 若 a 的溢出内容在阶段一保存并最后读取，之后 b 才开始需要保存，二者在满足大小和对齐等条件时可以复用同一栈槽。若 a、b 的保存需求重叠，就不能共享，否则 b 的 store 会覆盖 a 的未来 reload。

因此需要分析的是栈中内容的生命周期，而不只看虚拟寄存器区间是否重叠。一个值可能已经从寄存器消失，却仍要从溢出槽再次读取；也可能寄存器中仍有值，但栈中旧副本已永不使用。第 9 章 StackColoring 处理源层局部栈对象生命周期，本节 StackSlotColoring 处理分配产生的槽，两者都利用不重叠，但输入与阶段不同。
...
共享的前提是 b/c 栈中内容的活跃范围不重叠，并满足大小、对齐及目标限制。箭头表示槽映射，不是执行顺序。
...
**用两寄存器的小例子看快速分配的局部决策。** 假设一块中依次定义 a、b，然后 use a，再定义 c，最后 use b、use c，且 a 在那次 use 后死亡。简单分配可以先 a→R0、b→R1；处理完 use a 后释放 R0；c 就复用 R0，全程无需 spill。

现在把最后再加一次 use a。到定义 c 时，a、b 都还活跃，必须保存某个值到栈或使用其他合法策略。这里的冲突来自第三个值进入时两位置仍被占用，和一开始选 R0 还是 R1 并无本质关系。Fast 分配器通过局部状态快速管理这些保存、重载和物理位置，不要求先完成与全局优化分配器相同的一整套区间策略。

| 位置 | 原例的占用 | 增加末尾 use a 后 |
| --- | --- | --- |
| 定义 a 后 | R0=a | R0=a |
| 定义 b 后 | R0=a，R1=b | R0=a，R1=b |
| 首次 use a 后 | R0 可释放，R1=b | R0=a，R1=b |
| 定义 c | R0=c，R1=b | 需要为 a/b/c 的保存需求解决冲突 |

表按程序正向描述存储需求，便于理解；它不是宣称 LLVM 内部必须按这张表的扫描方向或编号选择。下面源码和 dump 才用于核对具体实现状态。
...
**从“候选物理寄存器”出发检查整个区间。** 设待分配 v 活跃于 [2,9)，R0 已有 a:[1,4)，R1 已有 b:[6,10)。检查 R0，重叠 [2,4)；检查 R1，重叠 [6,9)，所以没有直接可用位置。不是说现在时刻 2 的 R1 空闲，就能把整个 v 永久放入 R1。

接下来比较能否移走某个可溢出的干涉区间。若 a 的溢出代价很低且满足实现条件，可以让 a 让出 R0，再把 v 放入 R0；a 需要经过 spill/reload 或重新物化，并产生后续分配工作。若 R0 的干涉来自固定物理寄存器使用或不能跨越的 call regmask，则不能简单“溢出这个物理寄存器约束”。

这个过程解释了 Basic 为何需要区间、干涉查询和重新入队机制。它不等同于每个时刻选一只空闲寄存器，也不具备 Greedy 那样丰富的区域拆分策略。下面的实际例子应按“哪个区间入队 → 按什么顺序试位置 → 被谁挡住 → 哪个值改写并重新入队”四步阅读。
...
```mermaid
flowchart TD
 Q["取一个待分配区间"] --> A{"有空闲候选位置？"}
 A -->|有| D["分配并记录"]
 A -->|无| E{"可合法剔除干涉区间？"}
 E -->|是| V["撤销干涉分配并重新入队<br/>当前区间获得位置"]
 E -->|否| S["按当前阶段尝试拆分或推进阶段"]
 S --> T{"结果"}
 T -->|需重试| N["新区间或原区间入队"]
 T -->|无可用拆分| R["有条件的最后重着色<br/>或 spill / remat"]
 R -->|成功或产生新工作| Next["继续处理队列；队列空则完成"]
 R -->|不可分配| Fail["诊断并退出"]
 D --> Next
 V --> Next
 N --> Next
...
这是决策主干，正常停止条件为队列清空；不可分配诊断会退出。细分 RS 阶段、限制与失败边界以相邻正文为准。
...
**为什么局部失败后不一定立刻 spill。** 上一个例子中 v:[2,9) 在 R0 上只与 a 的末段重叠，在 R1 上只与 b 的开段重叠。若把 v 分成 [2,5) 与 [5,9)，前半可放 R1，后半可放 R0，并在边界传递值。这里用简化区间和忽略边界约束的模型说明可能性；实际还要选择合法插入点和计算 COPY 成本。

若直接溢出 v，可能在它多次使用处反复 reload；若拆成两段，只多一次寄存器间传递，就可能更便宜。反过来，若 v 只在末尾使用一次，拆分引入的复制未必比一次 reload 更好。Greedy 增加的工作，是探索这些局部与跨块的保存方式，并利用频率、权重和阶段限制控制尝试成本。

“剔除”与“溢出”也要分开。剔除先撤销某个区间已有的寄存器分配，让它重新参加分配；它以后也许找到其他位置，未必马上产生栈访存。溢出才真正选择栈或相关重物化改写。读状态机时分清这两个动作，才能理解为何一个区间会多次出现在队列中。
...
**沿一次定义、三次使用画出 gap。** 设 v 在位置 0 定义，位置 3、7、10 使用。相关位置之间形成 Gap1=(0,3)、Gap2=(3,7)、Gap3=(7,10)。它们的端点来自 v 的定义/使用，不是函数里每两条相邻机器指令。

假设某候选 R1 在 Gap1 与 V1 干涉，在 Gap2 空闲，在 Gap3 与 V2 干涉。分配器不只是问“哪个 gap 的长度最短”，而是问：把哪些连续 gap 组成一个新段后，该段的保存收益能否覆盖干涉代价及新增复制成本？空闲的 Gap2 可能与相邻段组合，但并不从数学上强制总要并给左侧。

```mermaid
flowchart LR
 D["Def v"] -->|"Gap1：候选 R1 与 V1 干涉"| U1["Use1 v"]
 U1 -->|"Gap2：R1 无该类干涉"| U2["Use2 v"]
 U2 -->|"Gap3：候选 R1 与 V2 干涉"| U3["Use3 v"]
```

若选择围绕最后使用拆出一个新值 v.tail，就要在合适位置把原值复制到 v.tail，改写最后使用，再分别计算两区间。复制本身也有输入、输出活跃性，不能把切点两边画成互不联系的线段。原书图 10-26 中的 COPY，就是把原值交给新片段的动作；本轮已对照 PDF 第 305 页核查这一图及相邻说明。
...
```mermaid
flowchart LR
 D["Def v"] -->|Gap1| U1["Use1"]
 U1 -->|Gap2| U2["Use2"]
 U2 -->|Gap3| U3["Use3"]
 V1["R1 上的 V1"] -. "造成 Gap1 干涉" .-> U1
 FREE["Gap2 无该干涉<br/>可探索相邻连续组合"] -.-> U2
 V2["R1 上的 V2"] -. "造成 Gap3 干涉" .-> U3
```

实线是 v 的相关定义/使用次序，虚线是对候选 R1 的干涉说明。图不按长度比例绘制；从 Gap1+Gap2 或 Gap3 中选择须比较实际新段收益，不能从图形位置直接决定。
...
**从一个二态能量式实际算两次更新。** 先用与 LLVM 三态实现分开的教学模型。A、B 各取 +1（偏向寄存器）或 −1（偏向栈），偏置 bA=2、bB=−1，连接权重 f=1，能量为 `E=−2A+B−AB`。偏置让 A 单独倾向寄存器、B 单独倾向栈；连接项则奖励两者选择相同状态，代表减少位置切换。

从 A=−1、B=−1 开始，E=`2−1−1=0`。固定 B=−1，A 的局部输入为 `bA+fB=1`，改 A=+1 后 E=`−2−1+1=−2`，能量下降。再看 B，局部输入为 `bB+fA=0`，在本例约定零时保持旧状态，因此不变。这一步没有强制两者最终相同：A 的寄存器偏好足够强，足以承担一处边界切换。
...
| +1 | −1 | −2 | A 的偏好收益覆盖切换代价 |
| +1 | +1 | −2 | 另一个同成本状态 |
| −1 | +1 | 4 | 两者都违背各自较强的局部倾向 |

这里穷举只有四种状态，能直接看到最小值；大型网络不能这样穷举，异步更新一般只能说明朝局部稳定状态移动。同步同时翻转多个节点，或者使用不同更新规则，不能直接套用单节点能量下降的论证。LLVM 的 SpillPlacement 还有三态、阈值和更新上限，下面会单独对应，避免把这个入门模型误当作源码的逐步执行记录。
...
```mermaid
flowchart LR
 A["节点 A: 状态 XA，偏置 bA"] ---|"权重 fAB"| B["节点 B: 状态 XB，偏置 bB"]
 B ---|"权重 fBC"| C["节点 C: 状态 XC，偏置 bC"]
 A ---|"权重 fAC"| C
```

边表示对称耦合，不是 CFG；缺少连接可用零权重表示。这里是经典离散二态模型的示意。
...
**先把网络输出翻译回程序边界。** 假设一个值进入热点循环 H，在 H 中反复使用，退出后进入很冷的 C。若让它在 H 的入口、出口都保留寄存器，循环体每次迭代就不必反复装载；如果只在冷侧转入栈，把切换放到低频边界，通常比在热点循环里多次切换更有利。

块边界选择不是彼此独立的。沿一条 CFG 边，前驱出口与后继入口描述的是同一次控制转移上的状态；边束用来把需要一致处理的边界组织起来。一个穿越块中的值如果入口、出口状态相同，可能直接贯穿；状态不同，则可能需要 reload、spill 或 COPY。块内有固定干涉时，即使两个边界都偏向寄存器，内部也未必能一直占住同一物理位置。

把一个候选结果转成拆分，可以按四步：先确定哪些边束倾向寄存器；再映射到各块的进入/离开状态；接着找到状态改变或干涉迫使改写的位置；最后实际创建新虚拟区间与复制/访存，并重新计算成本。网络回答的是某个候选的区域偏好，不能跳过最后一步直接宣布整个虚拟寄存器分配完成。
...
这是需要决定寄存器/栈区域的原始 CFG，不把每个基本块直接等同于 LLVM SpillPlacement 的网络节点。
...
```mermaid
flowchart TD
 E["entry<br/>入口边界 0，出口边界 1"] --> C["LoopCond<br/>入口 2，出口 3"]
 C --> R["ret<br/>入口 4，出口 5"]
 C --> B["LoopBody<br/>入口 6，出口 7"]
 B --> C
```

0～7 是块边界编号。沿 CFG 边关联相接边界，得到边束：{0}、{1,2,7}、{3,4,6}、{5}；边界编号不能直接当成四个边束的编号。
...
| 候选寄存器在块中的干涉 | 入口倾向 | 出口倾向 | 需要继续评估 |
| --- | --- | --- | --- |
| 覆盖两端及内部 | 栈 | 栈 | 是否改用其他候选或在使用附近重载 |
| 只靠近出口 | 寄存器 | 栈 | 在合法位置保存/切换 |
| 只在内部 | 寄存器 | 寄存器 | 内部仍需绕开冲突，不能假定全块驻留 |
| 只靠近入口 | 栈 | 寄存器 | 在冲突后合法重载/切换 |

该表表达候选区域约束的直觉；LLVM 还区分偏好、强制条件及实际使用位置。
...
```mermaid
flowchart TD
 E0["EB0: {边界0}"] --> S0["状态：由偏置与连接求得"]
 E1["EB1: {边界1,2,7}"] --> S1["entry 出口 / LoopCond 入口 / LoopBody 出口共享"]
 E2["EB2: {边界3,4,6}"] --> S2["LoopCond 出口 / ret 入口 / LoopBody 入口共享"]
 E3["EB3: {边界5}"] --> S3["ret 出口状态"]
```

本图只展开前一 CFG 的边束归属，不给未指定输入伪造偏置数值。LLVM 三态含义和迭代规则见正文，不能只用原图的 ±1 概括。
...
**先把 one-hot 向量还原成“选一项”。** 节点 i 有三个选项，向量 xi 可以是 [1,0,0]、[0,1,0] 或 [0,0,1]，只允许一项为 1。若节点成本 ci=[8,0,1]，分别选择三项时成本就是 8、0、1。`ciᵀxi` 只是从向量中挑中对应的一项，并不是把所有可用寄存器的成本相加。

两个节点还互相影响。矩阵 Cij 的行对应 i 的选项、列对应 j 的选项；i 选第 a 项，j 选第 b 项，成对成本就是 Cij[a,b]。一旦所有节点选项确定，总成本就是每个节点各取一项，再为每条边各取一个矩阵元素并求和。这一步算明白之后，公式中的向量与矩阵就只是紧凑记法。

对于寄存器分配，选项可以是溢出、R0、R1。若两个值干涉，双方同时选 R0 的矩阵元素为无穷，禁止这个组合；双方选不同寄存器可以是零。溢出代价另存在各自节点向量里。这就把“合法不合法”和“合法方案里谁更便宜”放进同一优化问题。
...
**完整算一个只有一只寄存器的实例。** x、y 同时活跃，都只有“栈、R0”两个选项，cx=[8,0]、cy=[3,0]，干涉矩阵为 `[[0,0],[0,∞]]`。行是 x，列是 y。四种选择的总代价如下：

| x 的位置 | y 的位置 | 节点成本加干涉成本 | 合法？ |
| --- | --- | --- | --- |
| 栈 | 栈 | 8+3+0=11 | 是 |
| 栈 | R0 | 8+0+0=8 | 是 |
| R0 | 栈 | 0+3+0=3 | 是 |
...
因此选 x→R0、y→栈，成本 3。原因是 x 的溢出代价更高，而不是 x 的名字排序靠前。若 x、y 不干涉，右下角可改为零，两者就能先后复用 R0，总成本变为零。这个变化清楚展示了干涉边在模型里的作用。

选项为“栈”仍要落实为后续 spill/reload，并为新产生的短期临时值分配位置。PBQP 解出一个选项向量，不等于已经生成最终机器指令；图可能因这些改写而重新建立并继续求解。
...
**R1 归约为什么必须等邻居确定后才回填。** 沿上节两节点例子消去 x：若 y 选栈，x 可选栈或 R0，条件最小成本是 min(8,0)=0；若 y 选 R0，x 只能选栈，条件最小成本是 min(8,∞)=8。把这两个条件结果加到 y 的成本 [3,0]，得到新成本 [3,8]。

现在图只剩 y，选择第一项栈，成本 3。反向恢复 x 时必须用“y 已选栈”这一列，所以 x 选 R0。若在消去 x 时不管 y 选什么就永久决定 x→R0，后来 y 也选 R0 就会冲突。R1 的本质是把被消节点的影响按邻居每一种选择保存下来，而不是提前猜一个位置。

R2 同理，只是要为两个邻居的每种选项组合保存条件最小成本，所以结果从一个向量变成一张新矩阵。若两邻居原来就有边，还要把新成本加到原矩阵上，避免丢失已有约束；回溯则依据两邻居最终选项恢复被消节点。下方原书数值例子沿用这种行列约定，便可逐项核算。
...
```mermaid
flowchart LR
 N0["独立节点 0<br/>成本向量 [2,0]"] --> A0["选索引 1，成本 0"]
 N1["独立节点 1<br/>成本向量 [2,4]"] --> A1["选索引 0，成本 2"]
```

无关联边时各节点独立取最小项；箭头是选择过程，不是新增 PBQP 边。
...
```mermaid
flowchart LR
 A["节点 0<br/>c0=[2,0]"] ---|"C01=[[2,1],[1,4]]"| B["节点 1<br/>c1=[2,4]"]
 B --- X["其余邻居"]
```

矩阵行属于节点 0，列属于节点 1；为节点 1 的每一选项，分别对节点 0 的选项取条件最小值。
...
```mermaid
flowchart LR
 A["保存归约信息<br/>节点 0 等待回溯"] -.-> B["节点 1<br/>新成本 [3,7]"]
 B --- X["其余邻居"]
```

实线为剩余 PBQP 边，虚线只说明回溯信息。节点 1 选项确定后，才能恢复节点 0 的条件最优选项。
...
```mermaid
flowchart LR
 A["节点 0"] ---|"C01=[[2,2],[1,4]]"| B["节点 1<br/>c1=[1,3]"]
 B ---|"C12=[[2,1],[1,4]]"| C["节点 2"]
```

先固定节点 0 与 2 的选项，再对节点 1 的选项取最小值。
...
```mermaid
flowchart LR
 A["节点 0"] ---|"新矩阵 D02=[[5,4],[4,3]]"| C["节点 2"]
 B["节点 1 的归约记录"] -. "根据 0、2 的最终选项回溯" .-> A
```

若原来已有 0–2 边，应把 D02 逐项加入其成本，而不是覆盖原边。
...
无向边表示需要成对成本矩阵的干涉；X 与 Z 没有干涉边，可以在允许时复用位置。每条边用正文的三选项矩阵。
...
**从干涉图推到两个颜色不够的原因。** 若 a、b、c 两两同时活跃，就形成三角形；只有 R0/R1 两种颜色时，a 用 R0、b 用 R1 后，c 与两者都相邻，没有合法颜色。必须通过溢出、拆分或其他变换改变问题。若图是链 a—b—c，a 和 c 不相邻，就可以都用 R0，b 用 R1。

```mermaid
flowchart LR
 subgraph T["三角形：两种颜色不足"]
 A["a"] --- B["b"]
 B --- C["c"]
 C --- A
 end
...
经典简化法的直觉是：有 K 种颜色时，若某节点度小于 K，先移走它；等邻居着色后，至多占掉 K−1 种颜色，至少还剩一种给它。这个保证来自邻居数量，而不是节点在图上画得靠边。实际寄存器类、预着色和别名会改变可选颜色集合，不能不加条件地用普通 K 色图替代全部机器约束。
...
a-b-c 构成三角形，故两色无法全部合法着色；三色时 b/d 不相邻，可以共用颜色 2。原图中的两色面板不能作为合法两色结果。

**图 10-49 图着色示意图**

现在要求相邻节点颜色不同。先给 a 颜色 1、b 颜色 2，c 同时邻接 a/b，所以两色已不够；增加颜色 3 给 c，再给 d 颜色 2 就得到图 10-49 的合法方案。因此该图三可着色，且不能只用两色。
...
```mermaid
flowchart TD
 G["建立干涉图及可用颜色约束"] --> L{"有可安全简化的低度节点？"}
 L -->|有| P["移除并压栈"]
 P --> L
 L -->|无且仍有节点| S["选择潜在 spill 候选并继续简化"]
 S --> L
 L -->|图空| C["逆序弹栈，为节点选择合法颜色"]
 C --> F{"能为所有节点着色？"}
 F -->|是| D["得到分配"]
 F -->|否| R["执行必要 spill / 改写，重建问题"]
 R --> G
```

图为经典图着色分配主干。潜在 spill 候选不一定最终 spill，反向选择时仍可能有可用颜色。
