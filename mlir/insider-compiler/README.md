...
- `<!-- source: ..., PDF p. N -->` 注释标示扫描来源页。同一清单因分页拆开的连续代码已合并；跨页处的来源注释按原顺序放在完整代码块之后，便于复制时保持代码连续。不同示例、转换前后及独立命令／输出保留各自代码块。原书已有的省略号和依赖外部定义的示意代码会明确标注。
- 代码以内容校对为主；对有疑点或涉及语义修正的地方做针对性验证，不要求每个清单都独立编译或执行。各章证据记录实际完成的检查范围。
