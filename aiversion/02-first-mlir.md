...
## 8. 为什么离开 AST 后仍保留 Toy 语义

AST 非常适合表达语法，但不同前端各自定义 AST，难以直接共享重写、诊断、验证和 pass 管理。LLVM IR 统一了低层表示，却不原生认识“张量转置”。Toy 方言在两者之间提供一个共同框架里的高层表示：统一的是操作骨架，仍可扩展的是语义。

例如把 `transpose(transpose(x))` 先变成若干内存循环，会失去一眼可见的恒等式。保留两个 `toy.transpose`，第 3 章只需一条结构匹配规则。多层表示的意义由这个具体例子就能看清：选择何时丢弃高层信息，是编译器设计的一部分。

## 9. Operation、Region、Block、Value 的层级

这几个概念最容易在第一遍阅读时混淆。Operation 是容器和语义单元，Region 组织嵌套控制/计算范围，Block 保存参数和有序操作，Value 是结果或块参数。一个函数可以是 Operation，函数体在 Region 中，函数入口是 Block，形参是该 Block 的参数。

```mermaid
flowchart TB
  M[ModuleOp] --> R[模块 Region 与 Block]
  R --> F[toy.FuncOp]
  F --> FR[函数 body Region]
  FR --> B[入口 Block]
  B --> P[Block arguments]
  B --> O[constant / transpose / mul]
  B --> T[toy.return]
```

图中的箭头表示包含关系，不表示执行顺序。若要看计算依赖，应另看 Value 的 use-def 链。MLIR 同时提供“嵌套结构”与“数据依赖”，这两张图不能混为一谈。

### 9.1 SSA 结果和源语言变量

对 `var a = [1,2]; var b = a;`，前端可把 a、b 都绑定到同一 Value；IR 不必为 b 分配新变量槽。每个 SSA Value 有唯一来源，但可有多个使用。如果某个 Value 来自 Block argument，它没有 defining Operation，因此 `getDefiningOp()` 可能返回空。

`%0`、`%a` 是打印名称。IR 重打印时它们可以重编号，但两个 use 是否引用同一个 Value 的事实不变。读测试应跟踪定义与使用关系。

### 9.2 Attribute、Type 与 Value

常量的数值放在 `DenseElementsAttr`，属性本身在编译期存在；`toy.constant` 把它作为一个操作的语义载荷，产生可由其他操作消费的 SSA Value。结果的 Type 是对这个 Value 的静态约束。三者的联系如下：

```mermaid
flowchart LR
  A[DenseElementsAttr: 数值与形状] --> O[toy.constant]
  T[TensorType: 结果类型] --> O
  O --> V[SSA Value]
  V --> U[toy.transpose 的 operand]
```

字典属性 `inplace=true` 在原文中只是解释 generic syntax 的例子；本地 TransposeOp 并没有实现“遇到它就原地修改内存”的运行语义。不能把可附加属性误认为已经提供了某项功能。

## 10. 从 ODS 到 C++：每一项声明在做什么

ODS 的 `arguments` 同时容纳 SSA operands 和 attributes，因此 ConstantOp 的 `arguments` 不为空，却仍然是 ZeroOperands。这不是矛盾：`F64ElementsAttr:$value` 不是 SSA 输入。

`results = (outs F64Tensor)` 让 ODS 知道结果数量为 1；由此推导的一些 Trait 不必再手写。`F64Tensor` 只约束张量元素类型，不要求固定维度，更不代表整套矩阵形状兼容检查都自动完成。

`hasVerifier = 1` 让生成类声明 `LogicalResult verify()`，开发者在 `.cpp` 中实现跨字段关系。`hasCustomAssemblyFormat = 1` 对应手写 parse/print；`assemblyFormat` 对应声明式生成。它们是两条选择路径，不应给同一个操作随意混配相冲突的定义。

### 10.1 Builder 与验证为何分开

Builder 让前端方便地创建常见合法操作，例如从稠密属性自动推导常量类型。但一个操作也能从文本解析或通过另一种显式构造接口进入 IR。因此 builder 中的方便推导不能代替 verifier。

本地标量 builder 创建 `RankedTensorType::get({}, f64)`，不是泛型 `getType(emptyShape)`。后者在 MLIRGen 中表示“声明没有形状信息”，会返回 UnrankedTensorType。相同的空列表，在不同 API 的语义上下文中可能不同，不能只看参数外观。

### 10.2 MLIRContext 为什么必须活得足够久

类型和属性在 Context 内唯一化，句柄可以便宜地拷贝和比较。Context 销毁后，依赖其中类型/属性/方言定义的 IR 不应继续使用。`OwningOpRef<ModuleOp>` 管理模块自身生命周期；它并不把 Context 一并包含在内部。

`OpBuilder` 另有插入点状态。函数开始时设为入口 Block 起始，生成表达式时顺序插入操作；进入另一层 Region 或临时插入全局对象时，应保存并恢复插入点。错误插入点会让一个逻辑上正确的操作出现在不允许的作用域。
...
这是按生成器行为重建的示意，省略 loc。Ch2 没有形状推断，即使调用点后来传入 2×3，也不会在本章自动把这个函数体专门化。

### 11.1 模块符号与局部变量不是同一张表

局部变量名字由 MLIRGen 的 ScopedHashTable 解析，结果是 Value。函数 callee 则保存在 SymbolRefAttr 中，引用 Module 里的符号操作。`@f` 与 `%arg0` 的语法差异反映了两种不同引用机制：符号引用不等于一个函数指针 SSA operand。

### 11.2 函数边界与 IsolatedFromAbove

`IsolatedFromAbove` 限制函数体不能隐式捕获外层 SSA Value。显式参数使函数输入清晰，也让函数级 pass 能独立处理其 Region。可引用模块符号并不意味着允许跨函数直接偷用另一个函数的 `%0`，二者是不同通道。

## 12. 验证、打印、往返：三种不同的成功

解析成功说明文本符合语法，验证成功说明操作满足已登记的不变量，运行成功才涉及后端和运行时。Round-trip 主要检查第一种表示转换是否可逆，以及 IR 是否仍能通过验证，不证明程序数学意义完全正确。
...
在允许未注册方言的工具模式中测试 generic 形式，可隔离“工具是否认识语义”的问题。对应标准工具选项为 `-allow-unregistered-dialect`；不要把 `mlir-opt` 当作自动链接了教学 Toy 方言的工具。教程的 Toy 注册在各章 `toyc-chN` 中。

## 13. 例题与解析

**为何打印出的常量比 ODS 中的约束更具体？** ODS 允许 F64Tensor，而 builder 根据属性给出实际的 RankedTensorType。这是“允许的集合”和“具体实例”的区别。

**为何函数 return 可以有有秩 operand，签名却无秩？** 本地 ReturnOp verifier 在两类型相同或任一侧无秩时放行，代表形状还未完全具体化。它不是 C/C++ 的常规强制类型转换规则。
