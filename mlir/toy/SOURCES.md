...
当前两套教材中 C++/TableGen/MLIR 块共 140 处：59 处逐字源码（36 段独立摘录，其中 23 段在两套正文中重复出现）、8 处译编、73 处示意，未分类 0 处。逐行比对只覆盖前 59 处；其余 81 处只检查分类、围栏及适用的文件引用，不代表其编译/运行正确性已验证。shell、Toy 源程序、text、Mermaid 块及独立教学输入不在这 140 处统计范围内。
...
2026-09-13 进一步细化 aiversion 第 0～7 章：新增 13 段有针对性的逐字摘录、25 组 shell 实验代码块和两个小型教学输入。official 正文未改动；各章新增部分均有说明和观察判据，入口见 [扩充教材目录](aiversion/README.md)。本轮没有为执行这些命令重建 LLVM，也没有将新输入标成已通过的运行测试。
