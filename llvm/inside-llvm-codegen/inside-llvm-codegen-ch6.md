...
**把 TableGen 看成生成记录的语言。** 普通 C++ 类在程序运行时创建对象；TableGen 的 `class` 在生成阶段提供带参数的记录模板，`def` 创建具体记录，`multiclass` 可以一次展开一组相互关联的记录，`defm` 实例化这组模板。最终是后端生成器读取展开后的字段，并输出 C++ 表格或函数。`.td` 文件本身不会在 `llc` 处理每条指令时重新执行。

例如一种加法指令的描述通常同时涉及结果寄存器类、两个输入、汇编打印格式、机器编码和匹配模式。它们分别回答“结果能放哪里”“怎样打印”“怎样编码”“什么计算能匹配”，并不互相替代。只写出字符串 `add` 不足以让选择器认识 `ISD::ADD`；有匹配模式也不自动保证可发射真实编码，某些记录本来就是 codegen pseudo。

阅读复杂 TD 时，可以先挑一个最终记录，例如本章的 ADD_rr，反向列出它继承的类和参数，再逐个展开 `let` 覆盖的字段。最后检查生成后的 record，确认看到的是最终值，而不是某个父类尚未被覆盖的默认值。原书的大量类定义与生成清单，目的就是让读者完成这一次“模板到实例”的追踪。
...
**一次生成与一次编译分别发生什么。** 构建 LLVM 时，`llvm-tblgen` 解析 include，实例化并解析记录，再由指定生成器输出文件。例如指令信息生成器输出 opcode/操作数相关表，DAG 指令选择生成器输出匹配程序；二者读取的都是目标描述，却服务于不同运行阶段。生成的 `.inc` 随后被 C++ 源文件包含并编译进工具。

运行 `llc` 时，已编译的选择器执行生成的匹配逻辑。它检查当前节点的 opcode、类型、操作数以及目标谓词，必要时失败回退到下一候选；目标手写选择逻辑还可以处理通用模式不便表达的情况。这样，TableGen 把重复的声明整理为实现，但并没有免除后端作者对合法化、调用约定、指令语义的责任。

出错时按阶段定位：语法/字段错误先看 TD 解析；记录存在而 matcher 缺失，看 Pattern 与生成选项；matcher 存在但运行时不命中，看合法化后的节点形状与谓词；选择成功而编码失败，再看 pseudo 展开、MC lowering 和编码字段。把这些问题都归为“TableGen 不生效”，会把排查范围扩大到整个后端。
...
```mermaid
flowchart LR
 TD["TD 文件与 include"] --> P["解析、实例化、字段解析"]
 P --> R["最终记录集合"]
 R --> L["llvm-tblgen 所选生成器"]
 R --> C["clang-tblgen 所选生成器"]
 R --> G["记录打印等通用生成器"]
 L --> I["寄存器 / 指令信息"]
 L --> M["指令选择 matcher 等"]
 C --> A["Clang AST / 诊断等描述"]
...
“后端”在这张图中指 TableGen 的代码生成器，不是运行时选择的 CPU 后端。一次调用按所选生成选项工作，不会自动生成所有种类输出。
...
```mermaid
flowchart LR
 A["输入 src: GPR 值"] --> N["ISD::ADD"]
 B["输入 src2: GPR 值"] --> N
 N --> Q["检查类型、模式和目标条件"]
 Q --> I["目标 ADD_rr<br/>输入 src/src2，定义 dst"]
 I --> U["原 add 的使用者改用 dst"]
```

实线按值从生产者流向使用者的方向画。匹配成功会创建目标操作，不是仅把 ADD 根删除并剩下一个没有定义的 dst。
...
**模式匹配依赖形状，也依赖约束。** 对 `add(x,constant)`，寄存器加立即数模式可能比寄存器加寄存器模式少一次物化常量，但前提是立即数满足位宽、符号扩展和编码限制。越界时，应走合法的其他形式；不能仅凭右操作数是 Constant 就选择立即数编码。可交换运算还可能需要考虑操作数交换，让常量落在模式期望的位置。

复杂地址模式同样如此：`base + offset` 看起来能合成寻址操作，但还要问 offset 能否编码、访问宽度如何、对齐有什么要求、地址计算是否被其他用户共享。共享节点若仍有其他使用，折叠一个使用不等于该计算已从整个函数删除。后面的 Pattern、PatFrag 与 ComplexPattern，可以分别按完整匹配、可复用子模式和需要目标代码参与的匹配来理解，再对照本章生成记录阅读。
