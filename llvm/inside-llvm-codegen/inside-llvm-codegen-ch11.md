...
**从抽象对象手算一个带对齐的布局。** 假设仅作教学用，栈向低地址增长，FP 是固定参考点，三个对象分别为 A：8 字节/8 字节对齐，B：4 字节/4 字节对齐，C：16 字节/16 字节对齐。可以先把 A 放 [FP−8,FP)，B 放 [FP−12,FP−8)，为了让 C 起点满足 16 字节对齐，把 [FP−16,FP−12) 留作填充，C 放 [FP−32,FP−16)。局部对象及填充共占 32 字节。
...
这不是任一具体 ABI 的完整栈帧，还没包含调用参数区、保存寄存器等。它说明为什么总大小未必等于对象大小简单相加，以及为什么 frame index 必须等布局确定后才变成“基址+偏移”。若访问 C 的第 8 个字节，其示例地址是 FP−24；目标若不能编码这个寻址形式，还要生成额外地址计算。

再把 callee-saved 加进来：函数用了某个被调用者负责保存的寄存器，就要在覆盖它之前保留旧值，在返回调用者前恢复。保存区域、局部对象、spill 槽、对齐与是否使用 FP 共同决定前后序。前言不是固定的两条 push，后序也不是把任意入口指令倒序执行；目标钩子需要按 ABI 和实际帧状态生成它们。
...
分配后的下沉比第 9 章更具体：操作数已经使用物理寄存器。假设入口有 `R1=COPY R0`，只有冷分支使用 R1。下沉可以让热分支少做一次复制，但必须检查从入口到冷分支新位置之间 R0 是否被改写；若有调用破坏 R0，移动后复制的就不是原值。

还要检查 R1 在移动区间中是否有其他使用或定义。虚拟 SSA 时一个名字的定义明确，分配后同一 R1 可能先后保存不同值，因此需要物理寄存器活跃性、别名和调用掩码。读本节例子时，把“复制的是哪个时刻的值”写在寄存器名旁边，再判断移动是否保持同一内容。
...
**沿两条完整路径检查保存与恢复是否配对。** 本节 getSqrt 例子只有 a>10 时调用 fun，其他情况直接返回零。普通布局在入口就保存返回地址、建立帧，于是两条路径都付出保存/恢复开销。若把保存移到调用分支开头、恢复放到该分支返回前，快速路径从头到尾都不触碰这份帧。

```mermaid
flowchart TD
 E["入口：比较 a 与 10"] -->|"a 不大于 10"| F["返回 0<br/>未保存，也不恢复"]
 E -->|"a 大于 10"| S["保存返回地址和所需寄存器"]
 S --> C["调用 fun：可能覆盖返回地址寄存器"]
 C --> R["恢复保存的状态"]
 R --> X["返回调用结果"]
```

推导合法性的第一步是找需要帧的操作，这里是普通调用所需的保存状态；第二步找支配所有这些操作的保存点；第三步检查每条经过保存的离开路径都先恢复；第四步保证没有未经过保存却进入恢复的路径。如果两条路径在返回前汇合，不能不加区分地让无帧路径也执行恢复。

循环会使条件更严格：若 Save 放进循环而 Restore 在循环外，多次保存一次恢复会破坏栈状态；即使能平衡，高频重复保存也可能比入口一次保存更贵。支配/后支配与块频率分别帮助解决路径覆盖与代价问题。最后还需更新 CFI，让调试器和异常展开器知道每个程序点的栈状态。
...
**先看两个尾部怎样变成一个共享尾部。** 原来 P 执行 `p专有代码; X; Y; ret`，Q 执行 `q专有代码; X; Y; ret`。若 X/Y/ret 在两路径上的物理操作数与语义都兼容，可以建立 T=`X;Y;ret`，P、Q 分别执行各自专有代码后跳到 T。静态少了一份后缀，但某路径可能新增跳转。
...
flowchart TD
 ...
 subgraph After["合并后"]
 P2["P 专有代码"] --> T["公共尾部 T: X; Y; ret"]
 Q2["Q 专有代码"] --> T
 end
```

第一遍比较最长相同后缀，第二遍确认活跃输入在进入 T 时确实相同且定义可用，第三遍修复跳转、块后继及活跃信息。文本上两条 ADD 相同不够；若隐式状态或寄存器中保存的内容不同，合并后的公共尾部也必须正确承接两条路径的值。

另一种常见折叠是移除跳到紧邻布局块的无条件分支。机器汇编可以利用 fallthrough，但还要确保实际布局和条件分支目标一致。不要反过来把这条机器层规则用于 LLVM IR：IR 仍要求显式 terminator，没有靠文本相邻推导出的隐式边。
...
把上图倒过来看：共享尾部 T 若很短，把它复制回前驱，可以减少进入 T 的跳转，并给布局或后续局部处理创造机会。尾合并与尾重复并不矛盾，它们在代码大小、跳转和路径信息之间选择不同的折中，且出现在不同阶段时拥有不同成本信息。

后期重复必须维护物理寄存器 live-in/live-out 和后继关系。如果 T 中有目标要求保持结构的分支、特殊调用或无法合法复制的操作，就不能因为指令数少而直接复制。比较第 9 章与本节时，应同时列出“输入是否仍为机器 SSA”和“哪些寄存器已经固定”，才能看出为何前期的 PHI 简化机会不能原样搬到后期。
...
**逐条走到阻止传播的位置。** `R1=COPY R0; use R1` 中，若 R0 在中间未改写、use 允许 R0，则可把 use 改用 R0。现在插入 `R0=其他值`：`R1=COPY R0; R0=其他值; use R1`。R1 保存旧 R0，新的 R0 已不是同一内容，传播必须在重定义处停止。

若改写的是 R0 的子寄存器，也可能改变整个寄存器的内容；调用的 regmask 也可能表示它被破坏。因此传播状态需要对物理寄存器别名和 clobber 失效。最后删 COPY 前，还要确认没有剩余显式/隐式使用依赖 R1。这个小例子体现了非 SSA 阶段不能仅靠名字相同判断值相同。
...
此时可以把 pseudo 展开放回完整过程：前面某条机器伪指令为了便于优化，把多条真实指令的含义暂时封装在一个操作里；到了目标规定的阶段，需要展开成真正能编码的序列。展开可能增加临时寄存器使用、隐式状态或访存，因此后续调度必须看到相应依赖。

如果一条伪指令只能在分配后知道物理操作数时展开，就不能强行提前；若展开需要新的虚拟寄存器，通常又需要安排在仍能处理它们的阶段。目标选择展开时机，就是在这些约束之间协调。阅读流水线时先问“这条 pseudo 为什么留到这里”，再看具体展开函数，能理解 Pass 次序并非任意排列。
...
**先把 CFG 与文件中的块顺序分开。** 假设 E 根据条件进入 H 或 C，H 是 90% 的热分支，C 是 10% 的冷分支，两者最终到 R。CFG 规定有哪些路径；布局决定这些块按 E,H,R,C 还是 E,C,R,H 等顺序放进地址空间。改变布局不应改变条件语义，但可能改变谁能 fallthrough、谁需要显式跳转。

若把热边 E→H 做成顺序直通，就可能让常走路径少一条转移。接下来希望 H→R 也连续，可以先形成链 E,H,R；冷 C 放到其他位置，再修正其跳转。块链策略的作用是保留已选的有利相邻关系，同时逐步合并更多块，而不是只按每块频率从高到低排序：两个很热但从不相连的块，紧挨着不一定有相同收益。

布局还影响条件分支方向、分支距离、对齐和指令缓存。读本节的图时，CFG 箭头表示控制可达，横向/纵向排列若表示布局，必须另行说明；不能因为重画位置就断言已经改变了机器代码的地址次序。
...
**把代码大小收益算成一笔账。** 假设一段重复序列长度为 L 字节，出现 n 次。原总大小为 nL。提取后保留一份公共体 L，每个位置换成 c 字节调用，再加 h 字节返回、对齐或其他固定开销，大小约为 `L+nc+h`。只有 `(n−1)L−nc−h` 为正，才在这个简化模型中减少大小。

举例 L=20、n=3、c=4、h=4，原来 60 字节，提取后 20+12+4=36，减少 24。若 L=4，其余相同，原来 12，提取后 20，反而变大。真实目标需要按实际指令大小和调用序列计算，这些数仅用于说明“发现重复”与“值得提取”之间还隔着成本判断。

合法性也必须成立：公共体的输入输出寄存器、调用破坏、返回地址与栈行为都要正确衔接；候选包含分支、不可移动指令或重叠出现时，还需专门处理。后缀树帮助高效找到重复序列，它并不自动证明这些序列可以安全变成子函数。
...
本节可以继续用 E/H/C/R 的函数理解。普通块布局把 C 放到函数末尾，仍属于同一函数体的地址范围；冷热分离则可能把冷块放进独立的冷代码区域，减少热代码中夹杂的冷指令，并让链接布局更集中地安排热路径。

分离不代表冷块不存在：它仍要从热区跳入，必要时跳回，读取原本可用的参数/栈状态，并保留异常处理与调试展开信息。于是必须处理跨区域分支、符号、重定位和相关元数据。仅将 Markdown 中两个块画远一点，不足以表达这些工程要求。

频率低也不等于可任意提成另一个普通函数。基本块拆分、函数抽取和机器公共代码提取的调用接口、适用阶段及目标不同。后面的工具实验应先确认采用哪一种变换，再比较布局、符号与实际增加的转移。
...
**本节的两次阅读顺序。** 先用下方 `aba$` 学会“插入后缀、共享前缀、在不匹配处分裂边”，再读 `abcabxabcd$` 的在线构造。这里补出长例子第一次真正拆边的过程，方便和后面的最终树对应：

| 已读前缀 | 发生的事情 | 为什么还要/不必拆边 |
| --- | --- | --- |
| a | 根建立 a 开头的叶边 | 没有已有首字符边 |
| ab | 原叶延长为 ab，另有 b 开头后缀 | 共享叶尾随新字符增长 |
| abc | 叶边继续延长，增加 c 开头后缀 | 三个后缀首字符不同 |
| abca | 后缀 a 已匹配已有 a 边的前缀 | 下一字符相同，可暂留隐式位置 |
| abcab | 后缀 ab 继续匹配已有边 | 活跃匹配长度增至 2 |
| abcabx | 已有 ab 后期待 c，新字符却是 x | 必须在 ab 后拆边；随后处理更短后缀 bx、x |

在读到 x 的阶段，首先把 a 开头的长边拆成公共 `ab`，它的两个孩子分别继续 `cabx` 与 `x`；再处理去掉首字符后的后缀，b 开头边也拆出公共 `b`，孩子分别继续 `cabx` 与 `x`；最后根增加 x 开头的叶边。新内部节点 ab 的后缀链接指向 b。此时输入还没有后面的 abcd$，所以边上的字符只是当前前缀，后续共享叶尾会继续延长它们。

这里“更短后缀”不是跳过一段任意 CFG 路径：它是在同一字符串中去掉一个开头字符后的后缀关系。后缀链接帮助算法从已识别的 ab 关系转到 b，skip/count 则避免重复扫描整条已匹配边。两种机制都为了减少构造工作，职责不同。

**从很短的 `aba$` 开始，亲手构造压缩树。** 所有后缀依次是位置 0 的 `aba$`、位置 1 的 `ba$`、位置 2 的 `a$`、位置 3 的 `$`。先插入 `aba$`，根只有一条通向叶 0 的边；插入 `ba$`，因为首字符不同，根增加 b 边；插入 `a$` 时与已有 a 边共享第一个 a，第二个字符却是 `$` 对 b，于是在第一个 a 后拆边；最后加入根的 `$` 边。
...
检查方法是从根到每个叶，把边字符串连起来：叶 0 得到 a+ba$，叶 2 得到 a+$，恰好是对应后缀。内部节点 a 下有两个叶，说明子串 a 出现两次，位置分别为 0、2。边 `ba$` 可以保存为原输入上的起止索引，不必复制三个字符到新数组里；这就是压缩树的空间共享。

再看在线构造为什么需要隐式位置。处理完 `ab` 后，读到第三个字符 a：后缀 `a` 已是某条已有边的前缀，暂时不必专门创建一个显式节点，可把活跃点停在边内。读到 `$` 时，已有边下一字符与 `$` 不符，才需要拆边并补叶。若每次都从根重新插入所有后缀，会反复走过已经匹配的字符；Ukkonen 的活跃点、共享叶尾和后缀链接正是为了避免重复工作。

原书较长的 `abcabxabcd$` 例子也可这样读：先在 a、b、c 阶段形成不同首字符的边；再次读到 a、b 时，活跃长度增长但不一定拆边；遇到 x，与预期 c 不同，才拆出 ab、b 等共享前缀并连接后缀关系。下面仍保留长例子的最终树与源码状态说明，先能验证每条根到叶路径，再跟踪活跃点会更清楚。
...
**从两个叶子推到两段机器代码。** 设指令序列被编码成整数 `[1,2,3,1,2,4]`，其中相同整数表示经过规则判定可视为相同的指令。根路径 `[1,2]` 若连接到后缀起点 0、3 的叶，就表示长度为 2 的候选在这两个位置各出现一次。路径长度给候选长度，叶的起点给替换位置。

下一步要回到原指令：整数相同是否正确考虑了 opcode、显式/隐式操作数等；两次出现有没有重叠；是否跨过不能提取的边界；公共输入输出能否满足目标调用约定；替换调用后的净字节收益是否为正。若在序列中插入独特的边界编码，可以阻止候选穿过不应跨越的位置。

比如 `[1,1,1]` 中长度 2 的子串在位置 0、1 都出现，但两段重叠，不能把这两次都当成互不影响的调用替换。重复次数高只是候选发现信息，合法的不重叠集合与目标成本才决定最终提取。因此后缀树负责搜索，MachineOutliner 还要负责候选筛选、成本和改写。
